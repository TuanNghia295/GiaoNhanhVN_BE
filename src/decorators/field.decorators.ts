...
import { ToBoolean, ToLowerCase, ToUpperCase } from '@/decorators/transform.decorators';
...
export function EnumArrayFieldOptional<TEnum extends object>(
  ...
  return applyDecorators(IsOptional(), EnumArrayField(getEnum, { required: false, ...options }));
}
...
export function FileFieldOptional(
  ...
  return applyDecorators(IsOptional(), FileField({ required: false, ...options }));
}
